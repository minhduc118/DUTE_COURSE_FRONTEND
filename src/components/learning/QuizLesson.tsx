...
export const QuizLesson: React.FC<QuizLessonProps> = ({
  ...
  const [attemptInfo, setAttemptInfo] = useState<QuizStartResponse | null>(
    null
  );
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [quizResults, setQuizResults] = useState<QuizAttemptResponse | null>(
    null
  );
  ...
  const handleStartQuiz = async () => {
    ...
    try {
      ...
      if (startResponse.maxAttemptReached) {
        alert(
          `Bạn đã hết số lần làm bài (${startResponse.usedAttempts}/${startResponse.maxAttempts}). Vui lòng liên hệ Admin.`
        );
        return;
      }
  ...
  const handleSubmitQuiz = useCallback(
    async (autoSubmit = false) => {
      console.log("handleSubmitQuiz called. AutoSubmit:", autoSubmit);

      if (!attemptInfo?.attemptId) {
        console.error("Missing attemptId. Cannot submit.", attemptInfo);
        alert(
          "Lỗi: Không tìm thấy thông tin lượt làm bài (Attempt ID). Vui lòng tải lại trang."
        );
        return;
      }
  ...
  if (!quizData)
    return (
      <div className="p-4 text-center">No quiz found for this lesson.</div>
    );
        ...
        <div className="quiz-container-focused">
          <div className="quiz-results-container p-5 text-center">
            <h2 className="mb-4">
              {quizResults.passed ? (
                <>
                  <i className="bi bi-check-circle-fill text-success"></i> Chúc
                  mừng! Bạn đã qua!
                </>
              ) : (
                <>
                  <i className="bi bi-x-circle-fill text-danger"></i> Chưa đạt
                  yêu cầu
                </>
              )}
            </h2>
            <div className="results-stats mb-4">
              <p>
                <strong>Điểm:</strong> {quizResults.score}
              </p>
              <p>
                <strong>Câu đúng:</strong> {quizResults.correctAnswers}/
                {quizResults.totalQuestions}
              </p>
              <p>
                <strong>Điểm kiếm được:</strong> {quizResults.earnedPoints}/
                {quizResults.totalPoints}
              </p>
              {quizResults.expired && (
                <p className="text-warning">⚠️ Bài nộp sau khi hết giờ</p>
              )}
            </div>
            <button
      ...
      <div className="quiz-lesson-view">
        <div className="quiz-container-focused p-5 text-center">
          ...
          <div className="quiz-info mb-4">
            ...
            <p>
              <i className="bi bi-clock"></i> <strong>Thời gian:</strong>{" "}
              {Math.ceil((quizData.timeLimitSeconds || 0) / 60)} phút
            </p>
            {quizData.passingScore && (
              <p>
                <i className="bi bi-trophy"></i> <strong>Điểm qua:</strong>{" "}
                {quizData.passingScore}
              </p>
            )}
            {quizData.maxAttempts && (
              <p>
                ...
                <strong>Số lần làm tối đa:</strong> {quizData.maxAttempts}
              </p>
            )}
          </div>
        ...
        <div className="quiz-status-bar">
          <div className="quiz-info-left">
            ...
            <h2 className="quiz-question-number">
              ...
              <span className="quiz-total-count">of {questions.length}</span>
            </h2>
          </div>
        ...
        <div className="quiz-main-content">
          ...
          <div className="quiz-options-grid">
            {currentQuestion.options?.map((opt: any) => (
              <label
                key={opt.optionId}
                className={`quiz-option-card ${selectedOptions.includes(opt.optionId) ? "selected" : ""
                  }`}
              >
                <input
                  type="checkbox"
                  ...
                  onChange={() =>
                    handleOptionSelect(currentQuestion.questionId, opt.optionId)
                  }
                />
        ...
        <div className="quiz-footer-actions">
          ...
          <div className="quiz-main-btns">
            ...
            <button className="btn-quiz-submit" onClick={handleNext}>
              ...
              <i
                className={`bi ${isLastQuestion ? "bi-check-lg" : "bi-arrow-right"
                  }`}
              ></i>
            </button>
          </div>
